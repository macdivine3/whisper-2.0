...
/**
 * THE SEAM. In L1 this returns a gentle canned reply. In L2, this same function
 * will call Claude with the conversation history + the Whisper system prompt.
 * Keep this signature stable so the swap touches nothing else.
 */
export async function getWhisperReply(
  history: ChatMessage[],
  mood?: string | null
): Promise<string> {
  // Seed off the conversation length so consecutive replies differ.
  const seed = history.length + (mood ? mood.length : 0);
  // Tiny delay is handled by the screen's typing indicator, not here.
